...
void setup()
{
    ...
    Wire.send(0x01); // wake it up
    Wire.endTransmission();
    Wire.beginTransmission(target);
    Wire.send(0x1C); //mode control accel
    Wire.send(0x00); //16G mode
    Wire.endTransmission();
    Wire.beginTransmission(target);
    Wire.send(0x1B); //mode control gyro
    Wire.send(0x00); //250 mode
    Wire.endTransmission();
    Wire.beginTransmission(target);
    Wire.send(0x6B);
    Wire.send(0x01);
...
void loop()
{
    ...
    if(Wire.available()){
      ...
      double xg = giveGyro(buffer[8], buffer[9], 'x');
      double yg = giveGyro(buffer[10], buffer[11], 'y');
      double zg = giveGyro(buffer[12], buffer[13], 'z');
      Serial.print(xg);
      Serial.print("\t");
      Serial.print(yg);
      Serial.print("\t");
      ...
      */
      ...
      if(forceMag > .5 && forceMag < 2) {
        ...
        pitch = pitch * .98 + abs(pitchAcc * .02);

        double rollAcc = atan2(x,z) * 57.29577;
        roll = roll * .98 + abs(rollAcc * .02);
      }
...
double giveGyro(uint8_t high, uint8_t low, char axis) {
  int16_t raw = ((high << 8) + low);
  int16_t compensated;
  if(axis == 'x') {
